...
//   // NEW: track image loading (only base frame to feel snappy)
...
//           stickers.funImage ||
//           activeFrameColor.imageUrl,
//     [mode, stickers, activeFrameColor]
//   );
...
//   // --------------------- Loader control (base image only) ---------------------
//   useEffect(() => {
//     if (!baseBikeImage) {
//       setImgLoadingCount(0);
//       return;
//     }
//     setImgLoadingCount(1);
//   }, [baseBikeImage]);

//   const handleImgLoaded = () => {
//     setImgLoadingCount((prev) => (prev > 0 ? prev - 1 : 0));
//   };
...
//                   title={c.colorName}
//                   onClick={() => {
//                     if (idx !== frameColorIndex) setFrameColorIndex(idx);
//                   }}
//                 />
//               ))}
//             </div>
...
//   // Base WebP image – ONLY sportyImage or funImage
//   const baseBikeImage = useMemo(() => {
//     const url =
//       mode === "sporty"
//         ? stickers.sportyImage
...
//     return (
//       <div style={pageWrapper}>
//         <div>No colours found with {mode} images. Please check data.</div>
//       </div>
//     );
//   }

//   // ---------- Canvas download (same WebP as preview) ----------
//   const loadImage = (src) =>
//     new Promise((resolve) => {
...
//       if (!baseImgEl) {
//         alert("Unable to load base image for download");
//         return;
//       }
            ...
            <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
              {filteredFrameColors.map((c, idx) => (
                <div
                  ...
                  style={colorDot(c.colorCode, idx === frameColorIndex)}
                  title={c.colorName}
                  onClick={() => idx !== frameColorIndex && setFrameColorIndex(idx)}
                />
              ))}
            </div>
          </div>

          <button style={primaryButton} onClick={handleCustomize} disabled={saving}>
            {saving ? "SAVING..." : "CUSTOMIZE"}
          </button>
        </div>
      </div>
